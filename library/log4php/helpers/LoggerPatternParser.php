...
class LoggerPatternParser
{
    ...
    public function parse()
    {
        ...
        foreach ($matches[0] as $key => $item) {
            ...
            if (empty($matches['word'][$key]) === false) {
                $word = $matches['word'][$key][0];
            } else {
                $word = null;
            }//end if
            
            if (empty($matches['modifiers'][$key]) === false) {
                $modifiers = $matches['modifiers'][$key][0];
            } else {
                $modifiers = null;
            }//end if
            
            if (empty($matches['option'][$key]) === false) {
    ...
    private function addConverter($word, $modifiers, $option)
    {
        ...
        if (isset($this->converterMap[$word]) === true) {
    ...
    {
        if (isset($this->converterMap[$word]) === false) {
