...
class LoggerConfiguratorDefault implements LoggerConfigurator
{
    ...
    {
        if (isset($config['threshold']) === true) {
            
            $threshold = LoggerLevel::toLevel($config['threshold']);
            
            if (isset($threshold) === true) {
        ...
        if ((isset($config['appenders']) === true) && (is_array($config['appenders']) === true)) {
        ...
        if (isset($config['rootLogger']) === true) {
            $this->configureRootLogger($hierarchy, $config['rootLogger']);
        }//end if
        
        // Configure loggers.
        if ((isset($config['loggers']) === true) && (is_array($config['loggers']) === true)) {
        ...
        // Configure renderers.
        if ((isset($config['renderers']) === true) && (is_array($config['renderers']) === true)) {
            foreach ($config['renderers'] as $rendererConfig) {
                $this->configureRenderer($hierarchy, $rendererConfig);
            }//end foreach
        }//end if
        
        if (isset($config['defaultRenderer']) === true) {
    ...
    private function configureRenderer(LoggerHierarchy $hierarchy, array $config)
    {
        ...
        if (empty($config['renderingClass']) === true) {
            $this->warn('Rendering class not specified. Skipping renderer definition.');
            return;
        }//end if
        
        if (empty($config['renderedClass']) === true) {
    ...
    private function configureAppender($name, array $config)
    {
        ...
        if (($appender instanceof LoggerAppender) === false) {
            $log  = 'Invalid class ['.$class.'] given for appender ['.$name.']';
        ...
        if (isset($config['threshold']) === true) {
            $threshold = LoggerLevel::toLevel($config['threshold']);
            if (($threshold instanceof LoggerLevel) === true) {
        ...
        if (($appender->requiresLayout() === true) && (isset($config['layout']) === true)) {
            $this->createAppenderLayout($appender, $config['layout']);
        }//end if
        
        // Parse filters.
        if ((isset($config['filters']) === true) && (is_array($config['filters']) === true)) {
    ...
    private function createAppenderLayout(LoggerAppender $appender, array $config)
    {
        ...
        $layout = new $class();
        if (($layout instanceof LoggerLayout) === false) {
    ...
    private function createAppenderFilter(LoggerAppender $appender, array $config)
    {
        ...
        $filter = new $class();
        if (($filter instanceof LoggerFilter) === false) {
    ...
    private function configureLogger(Logger $logger, array $config)
    {
        ...
        if (isset($config['appenders']) === true) {
            foreach ($config['appenders'] as $appenderName) {
                if (isset($this->appenders[$appenderName]) === true) {
        ...
        if (isset($config['additivity']) === true) {
            try {
    ...
    private function setOptions($object, array $options)
    {
        foreach ($options as $name => $value) {
            $setter = 'set'.$name;
            if (method_exists($object, $setter) === true) {
                $object->$setter($value);
            } else {
